...
def train(params):
    assert params["mode"].lower() == "train", "change training mode to 'train'"   # 检查是否为训练模式

    vocab = Vocab(params["vocab_path"], params["vocab_size"])    # 传入词典路径和词典大小  30000
...
def test(params):
    ...
    if params['greedy_decode'] and params["decoder_mode"] == "greedy_decode":
        # params['batch_size'] = 512
        predict_result(model, params, vocab, params['test_save_dir'], "greedy_decode")
    elif params['beam_search_decode'] and params["decoder_mode"] == "beam_search_decode":
        # 使用beam search进行搜索
        predict_result(model, params, vocab, params['test_save_dir'], "beam_search_decode")


def predict_result(model, params, vocab, result_save_path, decoder_mode):
    dataset = batcher(vocab, params)
    # 预测结果
    if decoder_mode == "greedy_decode":
        results = greedy_decode(model, dataset, vocab, params)
    elif decoder_mode == "beam_search_decode":
        results = beam_decode(model, dataset, vocab, params)
    else:
        raise Exception("decoder mode is not support!")
