...
def beam_decode(model, batch, vocab, params):
    ...
    enc_outputs, state = model.call_encoder(batch[0]["enc_input"])    # 第一次编码
    ...
    while steps < params['max_dec_steps'] and len(results) < params['beam_size']:
        ...
        dec_input = tf.expand_dims(latest_tokens, axis=1)  # shape=(3, 1)   # 解码输入
        # print('dec_input is ', dec_input)
        # print('step is ', steps)
        # print('dec_input is ', dec_input)
        # print('states is ', states)
        dec_states = tf.stack(states, axis=0)           # stack state
        ...
        returns = decode_onestep(batch[0]['enc_input'],  # shape=(3, 115)
                                 enc_outputs,  # shape=(3, 115, 256)  句子encoder输出
                                 dec_input,  # shape=(3, 1)
