...
class Vocab:
    def __init__(self, vocab_file, max_size):
        ...
        with open(vocab_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if w in self.word2id:   # 检查重复性单词
                    raise Exception('Duplicated word in vocabulary file: %s' % w)

                self.word2id[w] = self.count
                self.id2word[self.count] = w
                self.count += 1
                if max_size != 0 and self.count >= max_size:      # 设置只提取30000条词典数据
    ...
    def word_to_id(self, word):    # 训练阶段进行转换使用
...
def example_generator(vocab, train_x_path, train_y_path, test_x_path, max_enc_len, max_dec_len, mode, batch_size):
    ...
    if mode == "train":
        ...
        for raw_record in train_dataset:
            ...
            article_words = article.split()[:max_enc_len]    # 对文本进行分割处理,直接选取到最大长度的文本量即可
            enc_len = len(article_words)
            # 添加mark标记
            sample_encoder_pad_mask = [1 for _ in range(enc_len)]               # 添加mark标记

            enc_input = [vocab.word_to_id(w) for w in article_words]            # 单词转换为索引编号,不存在的单词直接设置索引为0
            enc_input_extend_vocab, article_oovs = article_to_ids(article_words, vocab)

            abstract_sentences = [""]    # 摘要文本
            ...
            dec_input, target = get_dec_inp_targ_seqs(abs_ids, max_dec_len, start_decoding, stop_decoding)   # 对摘要信息进行编码
            ...
            output = {
                "enc_len": enc_len,         # encode长度
                "enc_input": enc_input,     # 文本输入,索引编号输入方式
                "enc_input_extend_vocab": enc_input_extend_vocab,   # 对vocab进行扩展
                "article_oovs": article_oovs,       # 对不存在词典中的词存放至oov列表中
                "dec_input": dec_input,
                "target": target,
                "dec_len": dec_len,
                "article": article,
    ...
    if mode == "test":
        test_dataset = tf.data.TextLineDataset(test_x_path)   # 加载测试集数据
...
def batcher(vocab, hpm):
    dataset = batch_generator(example_generator, vocab, hpm["train_seg_x_dir"], hpm["train_seg_y_dir"],
                              hpm["test_seg_x_dir"], hpm["max_enc_len"],       # 200, 40
                              hpm["max_dec_len"], hpm["batch_size"], hpm["mode"])   # 训练文件等所在路径,最长的enc_len,dec_len,batch_siz
