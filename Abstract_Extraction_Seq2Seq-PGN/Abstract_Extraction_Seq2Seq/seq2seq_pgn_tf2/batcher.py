...
def article_to_ids(article_words, vocab):
    ...
    for w in article_words:
        ...
        if i == unk_id:  # If w is OOV
            ...
            ids.append(i)
    return ids, oovs     # oovs是类似这样的oovs单词  ids=文本的长度里面不存在用UNKNOWN_TOKEN代替的次了
...
def example_generator(vocab, train_x_path, train_y_path, test_x_path, max_enc_len, max_dec_len, mode, batch_size):
    ...
    if mode == "train":
        ...
        for raw_record in train_dataset:
            ...
            enc_input = [vocab.word_to_id(w) for w in article_words]   # 编码之后的输入-转换为index
            enc_input_extend_vocab, article_oovs = article_to_ids(article_words, vocab)

            abstract_sentences = [""]            # 要生成的句子
            abstract_words = abstract.split()
            abs_ids = [vocab.word_to_id(w) for w in abstract_words]
            abs_ids_extend_vocab = abstract_to_ids(abstract_words, vocab, article_oovs)   # 对摘要进行处理
            ...
            output = {
                "enc_len": enc_len,         # encoder输入长度
                "enc_input": enc_input,     # encoder输入index
                "enc_input_extend_vocab": enc_input_extend_vocab,         # 使用oovs列表中的索引代替原本的UNKNOWN_TOKEN
                "article_oovs": article_oovs,    # oovs单词列表
                "dec_input": dec_input,   # 解码输入
                "target": target,
                "dec_len": dec_len,
                "article": article,
                "abstract": abstract,
